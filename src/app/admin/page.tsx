...
export default function AdminEntryPage() {
  ...
  async function handleEnter() {
    if (code.trim().length < 6) { setError("Enter a 6-character party code"); return; }
    setLoading(true);
    setError("");
    try {
      ...
      setError(e instanceof Error ? e.message : "Party not found");
    } finally {
      setLoading(false);
    }
  }
    ...
    <main className="min-h-screen flex flex-col items-center justify-center p-6">
      <div className="bg-white rounded-3xl shadow-xl p-8 w-full max-w-sm animate-fade-in">
        <div className="text-center mb-6">
          ...
          <h1 className="text-2xl font-bold text-rose-600">Admin Login</h1>
          <p className="text-rose-400 text-sm mt-1">Enter your party code to continue</p>
        </div>
        ...
        <div className="space-y-4">
          ...
          >
            {loading ? "Finding party..." : "Go to Dashboard →"}
          </button>
        </div>
